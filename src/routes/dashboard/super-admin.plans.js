...
function SuperAdminPlans() {
    ...
    const [selectedGymForOverride, setSelectedGymForOverride] = useState(null);
    ...
    const filteredGymsForSearch = gyms?.filter(gym => gym.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const handleUpdatePlan = async () => {
        ...
        try {
            ...
            const cleanedFeatures = manualFeatures.filter((f) => f.trim() !== '');
            const planData = {
    ...
    const { data: gymsWithOverrides } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (error)
                throw error;
            return data?.filter(g => g.settings?.manual_pricing !== undefined) || [];
        }
    });
    return (<div className="flex-1 overflow-y-auto h-full w-full relative z-10 pb-24 md:pb-8">
      {/* Atmospheric Glow */}
      ...
      <div className="max-w-[480px] mx-auto w-full pt-12 px-5 md:px-0 relative z-10">
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <button data-testid="add-new-plan-btn" onClick={() => {
          ...
          <div className="space-y-3">
            {isLoadingPlans ? (<div className="space-y-3">
                {[1, 2].map(i => (<div key={i} className="h-48 bg-[#121411] border border-white/5 rounded-xl animate-pulse"></div>))}
              </div>) : globalPlans?.map((plan) => (<div key={plan.id} className={`bg-[#121411] border border-white/5 rounded-xl p-4 relative overflow-hidden group ${plan.name.toLowerCase() === 'unlimited' ? 'bg-[#c9f232]/10 border-[#c9f232]/30' : ''}`}>
                {plan.name.toLowerCase() === 'unlimited' && (<div className="absolute inset-0 bg-gradient-to-br from-[#c9f232]/20 to-transparent pointer-events-none"></div>)}
                ...
                <div className="flex justify-between items-start mb-4 relative z-10">
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${plan.name.toLowerCase() === 'unlimited'
                ? 'bg-[#c9f232] shadow-[0_0_15px_rgba(201,242,50,0.3)]'
                : 'bg-[#1e201d] border border-white/10'}`}>
                    <span className={`material-symbols-outlined ${plan.name.toLowerCase() === 'unlimited' ? 'text-[#576c00]' : 'text-[#C0C2B8]'}`} style={{ fontVariationSettings: "'FILL' 1" }}>
                ...
                <ul className="space-y-2 mb-6 relative z-10">
                  {plan.features?.map((feature, idx) => (<li key={idx} className={`flex items-center gap-3 text-sm ${plan.name.toLowerCase() === 'unlimited' ? 'text-[#e3e3dd]' : 'text-[#C0C2B8]'}`}>
                      <span className="material-symbols-outlined text-[#c9f232] text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                      {feature}
                    </li>))}
                </ul>

                <button onClick={() => handleEditPlan({ ...plan, price: (plan.price / 100).toString() })} className={`w-full h-12 text-xs font-bold rounded-full transition-colors flex items-center justify-center gap-2 relative z-10 ${plan.name.toLowerCase() === 'unlimited'
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setIsAddingOverride(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#c9f232]/10 border border-[#c9f232]/30 rounded-full text-[#c9f232] text-[10px] font-bold uppercase tracking-wider hover:bg-[#c9f232]/20 transition-colors">
              <span className="material-symbols-outlined text-[14px]">add</span>
              Add Manual Pricing
            </button>
          </div>
          <div className="space-y-3">
            {gymsWithOverrides?.length === 0 && (<div className="bg-[#121411] border border-white/5 rounded-xl p-8 text-center">
            ...
            {gymsWithOverrides?.map((gym) => (<div key={gym.id} className="bg-[#121411] border border-white/5 rounded-xl p-4">
                ...
                <div className="space-y-4 mb-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input className="w-full h-12 bg-[#1a1c19] border border-white/10 rounded-xl pl-8 pr-4 text-white font-bold focus:border-[#c9f232] outline-none" defaultValue={gym.settings?.manual_pricing} type="number" onBlur={async (e) => {
                const newPrice = parseFloat(e.target.value);
                if (!isNaN(newPrice)) {
                    await supabase.from('gyms').update({
                        settings: { ...gym.settings, manual_pricing: newPrice }
                    }).eq('id', gym.id);
      ...
      {isAddingOverride && (<div className="fixed inset-0 z-[110] flex flex-col justify-end">
          ...
          <div className="relative bg-[#121411] border-t border-white/10 rounded-t-[32px] p-6 pb-24 w-full max-w-[480px] mx-auto animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-5">
              {!selectedGymForOverride ? (<div className="space-y-4">
                  <div className="relative">
                    <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#858A7D]">search</span>
                    <input autoFocus className="w-full h-12 bg-[#1e201d] border border-white/10 rounded-xl pl-10 pr-4 text-[#e3e3dd] focus:border-[#c9f232] outline-none transition-colors" placeholder="Search existing gym..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}/>
                  </div>
                  
                  <div className="max-h-[300px] overflow-y-auto space-y-2">
                    {filteredGymsForSearch?.map(gym => (<button key={gym.id} onClick={() => setSelectedGymForOverride(gym)} className="w-full p-3 bg-[#1e201d] border border-white/5 rounded-xl flex items-center justify-between text-left hover:border-[#c9f232]/30 transition-colors">
                        <div>
                ...
                </div>) : (<div className="space-y-5 animate-in fade-in duration-300">
                  <div className="bg-[#c9f232]/10 border border-[#c9f232]/20 rounded-xl p-4 flex items-center justify-between">
                    ...
                    <button onClick={() => setSelectedGymForOverride(null)} className="text-[#858A7D] hover:text-white">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input required type="number" placeholder="e.g. 750" className="w-full h-12 bg-[#1e201d] border border-white/10 rounded-xl pl-8 pr-4 text-[#e3e3dd] font-bold focus:border-[#c9f232] outline-none" value={customMonthlyPrice} onChange={e => setCustomMonthlyPrice(e.target.value)}/>
                    </div>
                  </div>

                  <div className="flex gap-3">
                    <button type="button" onClick={() => setIsAddingOverride(false)} className="flex-1 py-4 bg-[#1e201d] text-[#858A7D] text-[15px] font-bold rounded-2xl">
                      Cancel
                    </button>
                    <button onClick={handleAddOverride} disabled={isSubmitting || !customMonthlyPrice} className="flex-[2] py-4 bg-[#c9f232] text-black text-[15px] font-bold rounded-2xl shadow-[0_12px_24px_rgba(201,242,50,0.15)] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {/* Edit Plan Modal */}
      {isEditingPlan && selectedPlan && (<div className="fixed inset-0 z-[110] flex flex-col justify-end">
          ...
          <div className="relative bg-[#121411] border-t border-white/10 rounded-t-[32px] p-6 pb-24 w-full max-w-[480px] mx-auto animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input type="text" placeholder="e.g. Premium" className="w-full h-12 bg-[#1e201d] border border-white/10 rounded-xl px-4 text-[#e3e3dd] font-bold focus:border-[#c9f232] outline-none" value={selectedPlan.name} onChange={e => setSelectedPlan({ ...selectedPlan, name: e.target.value })}/>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" className="w-full h-12 bg-[#1e201d] border border-white/10 rounded-xl pl-8 pr-4 text-[#e3e3dd] font-bold focus:border-[#c9f232] outline-none" value={selectedPlan.price} onChange={e => setSelectedPlan({ ...selectedPlan, price: e.target.value })}/>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {manualFeatures.map((feature, idx) => (<div key={idx} className="flex items-center gap-3 bg-[#1e201d] border border-white/5 p-3 rounded-xl">
                      <span className="material-symbols-outlined text-[#c9f232] text-sm">check_circle</span>
                      <input type="text" className="text-sm text-[#e3e3dd] bg-transparent border-none outline-none flex-1" value={feature} placeholder="Feature name..." onChange={e => {
                  ...
                  <button onClick={() => setManualFeatures([...manualFeatures, ''])} className="w-full py-2 border border-dashed border-white/10 rounded-xl text-[10px] font-bold text-[#858A7D] uppercase tracking-wider hover:border-[#c9f232]/30 transition-colors">
                    + Add Feature
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setIsEditingPlan(false)} className="flex-1 py-4 bg-[#1e201d] text-[#858A7D] text-[15px] font-bold rounded-2xl">
                  Cancel
                </button>
                <button onClick={handleUpdatePlan} disabled={isSubmitting} className="flex-[2] py-4 bg-[#c9f232] text-black text-[15px] font-bold rounded-2xl shadow-[0_12px_24px_rgba(201,242,50,0.15)] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {isSubmitting && <span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin"></span>}
                  Save Changes
                </button>
              </div>
