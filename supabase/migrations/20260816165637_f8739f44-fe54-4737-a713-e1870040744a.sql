...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gyms' AND column_name = 'plan_tier') THEN
        ALTER TABLE public.gyms ADD COLUMN plan_tier TEXT DEFAULT 'free'; -- 'free', 'basic', 'standard'
    END IF;
...
-- Add subscription_started_at default if not present
ALTER TABLE public.gyms ALTER COLUMN subscription_started_at SET DEFAULT now();
